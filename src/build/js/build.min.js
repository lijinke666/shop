...
(function (window) {
    var flag = true;
    window.methodCover = {
        capacity: function capacity(msg, time) {
            var time = time || 1000;
        ...
        getParams: function getParams(getName) {
            var data = JSON.parse(sessionStorage.getItem(getName));
            return data;
        },
        go: function go(url, setNmae, param) {
            //跳转
    ...
    function init($d) {
        ...
        dom.find('.hw-close').on('click', function () {
            dom.fadeOut('fast');
        });

        dom.find('.hw-wrap').on('click', function (e) {
            e.stopPropagation();
        }).append($d);
